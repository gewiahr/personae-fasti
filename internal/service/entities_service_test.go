...
func (r *entitiesRepoStub) EditChar(_ context.Context, update *dto.CharUpdate, playerID, gameID int) (*domain.Char, error) {
	r.editCalled = true
	r.editGameID = gameID
	return &domain.Char{ID: update.ID, GameID: gameID, HiddenBy: hiddenBy(update.Hidden, playerID)}, nil
}

func (r *entitiesRepoStub) EditNPC(_ context.Context, update *dto.NPCUpdate, playerID, gameID int) (*domain.NPC, error) {
	r.editCalled = true
	r.editGameID = gameID
	return &domain.NPC{ID: update.ID, GameID: gameID, HiddenBy: hiddenBy(update.Hidden, playerID)}, nil
}
...
func (r *entitiesRepoStub) EditLocation(_ context.Context, update *dto.LocationUpdate, playerID, gameID int) (*domain.Location, error) {
	r.editCalled = true
	r.editGameID = gameID
	return &domain.Location{ID: update.ID, GameID: gameID, HiddenBy: hiddenBy(update.Hidden, playerID)}, nil
}
...
func TestEditEntityAllowsVisibleAndOwnHiddenContent(t *testing.T) {
	for _, hiddenByID := range []int{0, 12} {
		t.Run(map[bool]string{true: "own hidden", false: "visible"}[hiddenByID != 0], func(t *testing.T) {
			repository := &entitiesRepoStub{char: &domain.Char{ID: 4, GameID: 8, HiddenBy: hiddenByID}}
			service := NewEntitiesService(repository, nil)
			player := &domain.Player{ID: 12, CurrentGameID: 8}

			_, err := service.EditPlayerCurrentGameChar(context.Background(), player, &dto.CharUpdate{ID: 4})
			if err != nil {
...
func TestEditEntityRejectsContentHiddenByAnotherPlayerBeforeWrite(t *testing.T) {
	tests := []struct {
		...
		{
			name: "char",
			repo: &entitiesRepoStub{char: &domain.Char{ID: 1, GameID: 8, HiddenBy: 99}},
			edit: func(s *EntitiesService, p *domain.Player) error {
				_, err := s.EditPlayerCurrentGameChar(context.Background(), p, &dto.CharUpdate{ID: 1})
				return err
			},
		},
		{
			name: "npc",
			repo: &entitiesRepoStub{npc: &domain.NPC{ID: 2, GameID: 8, HiddenBy: 99}},
			edit: func(s *EntitiesService, p *domain.Player) error {
				_, err := s.EditPlayerCurrentGameNPC(context.Background(), p, &dto.NPCUpdate{ID: 2})
				return err
			},
		},
		{
			name: "location",
			repo: &entitiesRepoStub{location: &domain.Location{ID: 3, GameID: 8, HiddenBy: 99}},
			edit: func(s *EntitiesService, p *domain.Player) error {
				_, err := s.EditPlayerCurrentGameLocation(context.Background(), p, &dto.LocationUpdate{ID: 3})
				return err
			},
		},
	}
...
func TestEditEntityRejectsCrossGameIDBeforeWrite(t *testing.T) {
	repository := &entitiesRepoStub{char: &domain.Char{ID: 4, GameID: 9}}
	service := NewEntitiesService(repository, nil)
	player := &domain.Player{ID: 12, CurrentGameID: 8}
